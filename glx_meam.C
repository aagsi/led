...
// define prototype
//  qeon_20m_off_extrabinning.root
void tree1r(TString fileName, TH1F *hPTime[nmcp][npix]);
void glx_meam(TString in= "final_20_1_opt.root" , TString out_file= "out.root"){
	...
	for(Int_t m=0; m<nmcp; m++){
		...
		for(Int_t p=0; p<npix; p++){
			...
			//hPTime[m][p]  = new TH1F(Form("hPTime_%d",m*100+p),Form("mcp %d, pixel %d",m, p),200,4,8);//50
			hPTime[m][p]  = new TH1F(Form("hPTime_%d",m*100+p),Form("mcp %d, pixel %d",m, p),200,4.5,7);
...
///////////////////////////////////////////////////////////////////
	...
	TH1F * hPTimeError_1= new TH1F("timeError_1",";Time Error [ns];Entries [#]",50,0.0,0.140);
	TH1F * hPTime_histo_1= new TH1F("hPTime_histo_1",";T0 [ns];Entries [#]",50,4.5,7);
	//TH1F * hPTime_rms_1= new TH1F("hPTime_rms_1",";T0 rms [ns];Entries [#]",50,0.0,0.140);
	...
	for (Int_t t=0; t<n1; t++){
		chain1.GetEntry(t);
	if(nEntries > 60 ){
		hPTime[nMCP][nchannel-100*nMCP]->Fill(mean_g_fit);
		
		hPTime_histo_1->Fill(mean_histo);
		hPTimeError_1->Fill(error_mean_g_fit);
		//hPTime_rms_1->Fill(sigma_histo);
		
		}
	}
/*
TCanvas* c_hPTime_histo_1 = new TCanvas("c_hPTime_histo_1","c_hPTime_histo_1",800,600);
hPTime_histo_1->Draw();

TCanvas* c_hPTimeError_1 = new TCanvas("c_hPTimeError_1","c_hPTimeError_1",800,600);
hPTimeError_1->Draw();

TCanvas* c_hPTime_rms_1 = new TCanvas("c_hPTime_rms_1","c_hPTime_rms_1",800,600);
hPTime_rms_1->Draw();
*/
...
void tree1r(TString fileName, TH1F *hPTime[nmcp][npix]){
	...
	TH1F * hPTimeError= new TH1F("timeError",";Time Error [ns];Entries [#]",50,0.0,0.200);
	TH1F * hPTime_rms= new TH1F("hPTime_rms",";T0 rms [ns];Entries [#]",50,0.0,0.200);
	TH1F * hPTime_histo= new TH1F("hPTime_histo",";Mean T0 [ns];Entries [#]",50,4.5,7);
	...
	//for (Int_t m=64; m< 65; m++) {
		for (Int_t p=0; p< npix; p++) {
		//for (Int_t p=37; p< 38; p++) {
		...
		*/
			...
			if(hPTime[m][p]->GetEntries() == 1000 ){
				...
				//fhDigi[m]->Fill(row, col,sigma_g_fit);
				//fhDigi[m]->Fill(row, col,mean_histo);
				//fhDigi[m]->Fill(row, col, mean_g_fit);  
				
				hPTime_histo->Fill(mean_histo);
				hPTime_rms->Fill(sigma_histo);
				hPTimeError->Fill(mean_histo);
				
				//hPTimeError->Fill(mean_histo);
				//hPTimeError->Fill(mean_g_fit); 
	...
	hPTimeError->Draw();
	TCanvas* c_hPTime_histo = new TCanvas("c_hPTime_histo","c_hPTime_histo",800,600);
	hPTime_histo->Draw();
